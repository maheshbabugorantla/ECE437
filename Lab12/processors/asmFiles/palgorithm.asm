...
#Setup
mainp0:
  push  $ra                # push return addr
  ori   $t6, $zero, 10     # set max rands to 256
  ori   $t9, $zero, 5      # set seed to 5
  ori   $t7, $zero, 0      # reg 7 to 0 (counter for number of randoms generated)

# gen random number
genRand:
  
  or   $a0, $zero, $t9     # set argument to previous value (or seed if first loop)
  jal  crc32               # generate random num
  or   $t8, $zero, $v0     # set temp register to return value of crc32
...
mainp1:
  ...
  ori   $t5, $zero, 10
  ori   $t6, $zero, 0       # initialize temp reg 7 to zero/ counter for rand nums

  #initialize results
  ori   $s0, $zero, 0xFFFFFFFF  # Min val
  ori   $s1, $zero, 0x00000000  # Max val
  ori   $s2, $zero, 0           # sum of random vals 
